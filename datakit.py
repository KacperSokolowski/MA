...
def transform_data(main, only_expired, duration_start, duration_end, utilize_morf): 
    ...
    # Create: area, room_number, students_allowed, non_smoker_only
    ...
    # Transform approximate_coordinates
    df['approximate_coordinates'] = df['approximate_coordinates'].astype(int)
    ...
    'utilities', 'adv_description', 'days_difference',
...
def compute_average(indices, legacy_data):
    ...
    """
    if len(indices) > 0:
        return legacy_data.iloc[indices]['log_price_per_square'].mean()
    else:
        return 0
