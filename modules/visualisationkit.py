...
def plot_duration_histogram(df, convert_date):
    """
    Filter rows where 'expired' == 1, convert date columns,
    compute days difference, and plot a histogram.
    
    Parameters:
    ----------
    - df: pandas.DataFrame with columns 'expired', 'added_dt', 'expired_date'
    - convert_date: function to convert date strings to datetime
    ...
    # Keep only expired rows
    data = data[data.expired.eq(1)]
    
    # Convert date columns
    data['added_dt']     = data['added_dt'].apply(convert_date)
    data['expired_date'] = data['expired_date'].apply(convert_date)
    
    # Compute difference in days
    data['days_difference'] = (data['expired_date'] - data['added_dt']).dt.days
...
def plot_target_histogram(df):
    """
    Plot a histogram with KDE for the rent column of a DataFrame.
...
def plot_heat_map_district(
    ...
    """
    Plot a heat map of apartment model residuals by Warsaw district.

    Parameters:
    ----------
    df : pandas.DataFrame
        Data frame of apartment records. It must contain:
        - 'longitude': float longitude values
        - 'latitude': float latitude values
        - 'residual': numeric model residuals for each record
    recording_units_pth : str, optional
        File path to the shapefile (.shp) containing Poland’s recording units.
        The default points to 'geographic_data/recording_units_poland/...'.

    Returns:
    -------
    - None (displays a matplotlib heat map)
    """
    ...
    mean_residuals = districts_with_counts.groupby('index_right')['residual'].mean()
    districts['mean_residual'] = mean_residuals.fillna(0)
    column_to_plot = 'mean_residual'
    cmap = 'coolwarm'  
    ...
    # Plot the heat map with fixed range
    districts.plot(
        ...
        legend=True,
        alpha=0.6,
        legend_kwds={'label': "Residuals (PLN)", 'orientation': "vertical"}
    )
